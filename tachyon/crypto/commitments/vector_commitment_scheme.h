...
class VectorCommitmentScheme {
 public:
  ...
  // the size of |container| doesn't match with the size of parameters.
  ...
  [[nodiscard]] bool Commit(const Container& container, size_t index) {
    Derived* derived = static_cast<Derived*>(this);
    if (!batch_commitment_state_.batch_mode) return false;
  ...
  // |batch_mode| is false or the size of |container| doesn't match with the
  // size of parameters.
  template <typename T = Derived, typename Container,
            std::enable_if_t<
                VectorCommitmentSchemeTraits<T>::kSupportsBatchMode>* = nullptr>
