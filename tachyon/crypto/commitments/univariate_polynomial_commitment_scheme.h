...
class UnivariatePolynomialCommitmentScheme
 ...
 public:
  ...
  // |index| if |batch_mode| is true. Return false if |batch_mode| is false or
  // the degree of |poly| exceeds |kMaxDegree|.
  ...
  [[nodiscard]] bool Commit(const Poly& poly, size_t index) {
    ...
    if (derived->batch_commitment_state_.batch_mode) return false;
    return derived->DoCommit(poly, derived->batch_commitment_state_, index);
  }
  ...
  // |index| if |batch_mode| is true. Return false if |batch_mode| is false or
  // the degree of |evals| exceeds |kMaxDegree|.
  template <typename T = Derived, std::enable_if_t<VectorCommitmentSchemeTraits<
                                      T>::kSupportsBatchMode>* = nullptr>
  [[nodiscard]] bool CommitLagrange(const Evals& evals, size_t index) {
    Derived* derived = static_cast<Derived*>(this);
    if (derived->batch_commitment_state_.batch_mode) return false;
    return derived->DoCommitLagrange(evals, derived->batch_commitment_state_,
                                     index);
  }
};
